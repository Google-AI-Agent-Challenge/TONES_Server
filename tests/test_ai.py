...
@patch("app.services.ai_service.HAS_VERTEX_SDK", True)
@patch("app.services.ai_service._init_vertex_ai")
@patch("app.services.ai_service.GenerativeModel")
def test_vertex_generative_ai_answer(mock_gen_model, mock_init, client):
    ...
    # Mock Vertex AI GenerativeModel response
...
def test_pgvector_semantic_search_sql():
    ...
    # cursor execute SQL에 pgvector 코사인 거리 연산자(<=>) 및 filter가 바인딩되었는지 검증
...
async def test_cloud_sql_atomic_rollback_on_save_failure():
    ...
    dashboard_service = DashboardService(db_conn=mock_conn)
    ai_service = AIService(db_conn=mock_conn)
