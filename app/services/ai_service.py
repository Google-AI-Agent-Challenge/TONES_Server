...
class AIService:
    ...
    def generate_trend_briefing(self, this_week: dict, last_week: dict, product_name: str) -> str:
        ...
        if is_stable:
            system_instruction += (
                "\n\n[특별 지시사항]\n"
                "현재 성분, 제형, 용기의 3대 속성 변동폭이 매우 미미합니다. "
                "텍스트 내에 변동폭 수치나 0.0%p 등의 변동폭 정보를 일절 넣지 마십시오. "
                "대신 '품질 이슈 없이 견고하고 안정적인 만족도를 유지하고 있습니다'라는 표현을 포함하여 "
    ...
    def _local_trend_briefing_fallback(self, ing_diff: float, form_diff: float, cont_diff: float, rating_diff: float, product_name: str) -> str:
        ...
        if is_stable:
            ...
            trend_text = (
                ...
                "성분 및 피부 진정, 제형 및 발림성, 용기 편의성 등 모든 주요 속성에서 "
                f"품질 이슈 없이 견고하고 안정적인 만족도를 유지하고 있습니다. "
